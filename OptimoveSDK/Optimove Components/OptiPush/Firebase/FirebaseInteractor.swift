...
class FirebaseInteractor:NSObject
{
    ...
    private func setupAppController(_ appController: FirebaseOptions) {
        if UserInSession.shared.isClientHasFirebase {
    ...
    {
        if !UserInSession.shared.isClientUseFirebaseMessaging
        {
...
extension FirebaseInteractor:MessagingDelegate
{
    ...
    {
        if UserInSession.shared.isClientHasFirebase {
            guard UserInSession.shared.defaultFcmToken != fcmToken else {
            ...
            OptiLogger.debug("old fcm: \(String(describing: UserInSession.shared.fcmToken ?? ""))")
            retreiveFcmToken(for: optimoveAppSenderId) {[unowned self] (token) in
                ...
                UserInSession.shared.defaultFcmToken = fcmToken
            }
        } else {
    ...
    func subscribeToTopic(topic:String,didSucceed: ((Bool)->())? = nil)
    {
        if !UserInSession.shared.isClientHasFirebase {
            ...
            guard let fcm = UserInSession.shared.fcmToken else {return }
